...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference("Target.count") do
      post targets_url, params: { target: { describtion: @target.describtion, finish_date: @target.finish_date, start_date: @target.start_date, status: @target.status, team: @target.team, title: @target.title } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { describtion: @target.describtion, finish_date: @target.finish_date, start_date: @target.start_date, status: @target.status, team: @target.team, title: @target.title } }
